...
#define FWD(...) ::std::forward<decltype(__VA_ARGS__)>(__VA_ARGS__)
   ...
   *  converted to `Base_type *` at compile time. Notice how the templates of
  ...
  using is_base_of_template = decltype(is_base_of_template_I<Base_type>(std::declval<Derived_type *>()));

  namespace __private {
    class DockerClientImplBase {
    public:
      virtual std::string createContainer(OptionSetter &option) = 0;
      virtual void setAPIVersion(const std::string &api) = 0;
    };
  }
}
